import 'package:app_cripto/model/transposicion/transposicion_filas_model.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';

class FilasScreen extends StatefulWidget {
  static const name = 'filas_transposicion_screen';

  const FilasScreen({super.key});

  @override
  State<FilasScreen> createState() => _FilasScreenState();
}

class _FilasScreenState extends State<FilasScreen>
    with SingleTickerProviderStateMixin {
  final TextEditingController _textoController = TextEditingController();
  final TextEditingController _numFilasController = TextEditingController();
  final TextEditingController _rellenoController = TextEditingController();
  late TabController _tabController;
  late FilasTransposicion _cifrador;
  ...
  void initState() {
    ...
    _cifrador = FilasTransposicion();

    // Valor predeterminado para el carácter de relleno
    _rellenoController.text = 'X';

    // Validar parámetros cuando cambien
    _numFilasController.addListener(_validarParametros);
    _rellenoController.addListener(_validarParametros);
  }

  void _validarParametros() {
    setState(() {
      // Verificar que el número de filas sea válido (entero positivo)
      bool numFilasValido = false;
      if (_numFilasController.text.isNotEmpty) {
        try {
          int numFilas = int.parse(_numFilasController.text);
          numFilasValido = numFilas > 0;
        } catch (e) {
          numFilasValido = false;
        }
      }
      ...
      _parametrosValidos = numFilasValido && rellenoValido;
    });
  }

  void _procesarTexto() {
    if (_textoController.text.isEmpty || _numFilasController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('El texto y el número de filas no pueden estar vacíos'),
        ),
      );
      return;
    }
    ...
    if (!_parametrosValidos) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'El número de filas debe ser un entero positivo y el relleno un solo carácter',
          ),
        ),
      );
      return;
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'Ingresa el número de filas (ejemplo: 4)...',
                      border: OutlineInputBorder(
                        borderRadius: BorderRadius.circular(12),
                      ),
                      focusedBorder: OutlineInputBorder(
                      ...
                      fillColor: Colors.white,
                      helperText:
                          'Define el número de filas de la matriz de transposición',
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text(
                        'Este método organiza el texto en una matriz con un número específico de filas. El texto se escribe verticalmente (por columnas) y se lee horizontalmente (por filas) para obtener el texto cifrado. Para descifrar, se invierte el proceso.',
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                ...
                                Expanded(
                                  flex: 3,
                                  child: Text('QAREMEUFZAPXEUACAXLETOÑX'),
                                ),
                              ],
                            ),
                          ],
